...
description: "Deep Dive in die Protokolle rund um KI-Agenten – MCP, A2A, x402, AP2, ACP, UCP – im Vergleich mit REST, SOAP, GraphQL, gRPC und Webhooks. Mit Schichtenmodell, Steckbriefen, Entscheidungsbaum und einem Kapitel zu Authentifizierung und Agentic Payments."
pubDate: 2026-09-12
...
4. [MCP – der Agent und seine Tools](#4-mcp--der-agent-und-seine-tools)
...
7. [Agent und UI: AG-UI, A2UI und MCP Apps](#7-agent-und-ui-ag-ui-a2ui-und-mcp-apps)
8. [Agentic Payments und Agentic Commerce: x402, AP2, ACP, UCP](#8-agentic-payments-und-agentic-commerce-x402-ap2-acp-ucp)
...
3. **Das Protokoll im engeren Sinn** – was die Nachricht *bedeutet* und welche Abläufe erlaubt sind: Welche Operationen gibt es, welche States, welche Fehler, wie authentifiziert man sich.

Ein Beispiel macht es greifbar: MCP ist ein Protokoll. Es benutzt JSON-RPC 2.0 als Message-Konvention, JSON als Format und wahlweise stdio oder HTTP als Transport. Sagt jemand „wir nutzen JSON statt MCP“, vergleicht er Äpfel mit Fahrplänen. Die Aussage „wir nutzen REST statt MCP“ ist dagegen sinnvoll, denn beide liegen auf derselben Ebene: Beide beschreiben, wie ein Aufrufer mit einer Gegenstelle spricht.
...
<div class="stack" role="img" aria-label="Schichtenmodell: Von unten nach oben Netz (IP), Transport (TCP, QUIC), Sicherheit (TLS 1.3), Anwendungsprotokoll (HTTP/1.1, HTTP/2, HTTP/3, WebSocket, SSE, AMQP, MQTT, stdio), Message-Konvention (REST-Stil, SOAP-Envelope, JSON-RPC 2.0, GraphQL-Dokumente, gRPC), Format (JSON, XML, Protobuf, CBOR), fachliches Protokoll (MCP, A2A, AG-UI, A2UI, AP2, x402, ACP, UCP).">
  <div class="layer is-agentic">
    <div class="layer-name">Fachliches Protokoll <small>Semantik, Abläufe, States</small></div>
  ...
  <div class="layer is-classic">
    <div class="layer-name">Message-Konvention <small>Aufrufstil, Envelope</small></div>
    <div class="layer-items">
  ...
  <div class="layer is-base">
    <div class="layer-name">Anwendungsprotokoll <small>Transport</small></div>
    <div class="layer-items">
  ...
  <div class="layer is-base">
    <div class="layer-name">Netzwerk &amp; Verschlüsselung <small>Leitung</small></div>
    <div class="layer-items">
...
**Format.** JSON (RFC 8259) hat XML als Standardformat für Web-APIs praktisch abgelöst: leichter zu lesen, direkt in JavaScript nutzbar, ohne Namensräume und Schemata, die man erst laden muss.[^json] XML (W3C, 1998, fünfte Ausgabe 2008) lebt weiter, wo es formale Schemata, Signaturen auf Message-Ebene und jahrzehntealte Integrationen gibt – vor allem im SOAP-Umfeld.[^xml] Protocol Buffers sind binär, kompakt und mit Codegenerierung verbunden; man bezahlt mit Lesbarkeit und dem Zwang, beide Seiten zu kompilieren. CBOR (RFC 8949) ist die binäre Verwandte von JSON für Geräte mit wenig Ressourcen.[^cbor] Wichtig: **JSON ist kein Vertrag.** Was ein Feld bedeutet und welche Felder Pflicht sind, beschreibt erst ein Schema – bei REST OpenAPI, bei MCP JSON Schema 2020-12 für jede Tool-Eingabe.[^mcp-tools]

**Message-Konvention.** Hier entscheidet sich der *Stil* des Aufrufs. REST ist kein Protokoll, sondern ein Architekturstil aus Roy Fieldings Dissertation von 2000: Ressourcen mit URLs, die Standardverben von HTTP, stateless Aufrufe.[^fielding] JSON-RPC 2.0 ist das Gegenteil: ein winziger Envelope (`method`, `params`, `id`) für entfernte Prozeduraufrufe, transportunabhängig und seit 2010 unverändert – und genau deshalb die Basis von MCP und eine der drei Bindings von A2A.[^jsonrpc] SOAP legt einen XML-Envelope mit Header und Body um alles. gRPC bringt seine Konvention mit Protobuf-Servicedefinitionen und HTTP/2 gleich mit.

**Transport.** HTTP ist der Normalfall, mittlerweile in drei Generationen: HTTP/1.1, HTTP/2 mit Multiplexing über eine Verbindung (die Basis von gRPC) und HTTP/3 über QUIC statt TCP.[^http] WebSockets (RFC 6455) machen aus einer HTTP-Verbindung einen dauerhaften Kanal in beide Richtungen.[^ws] Server-Sent Events sind der einfache, nur in eine Richtung laufende Bruder – und werden von MCP und A2A für Streaming benutzt.[^sse] Message-Queue-Protokolle wie AMQP oder MQTT entkoppeln Sender und Empfänger zeitlich. Und **stdio**, also stdin und stdout eines lokal gestarteten Prozesses, ist ein vollwertiger Transport: MCP nutzt ihn für lokale Server, die direkt auf dem Rechner des Nutzers laufen.[^mcp-transports]

> **💡 Merksatz:** Format sagt, wie es aussieht. Transport sagt, wie es ankommt. Protokoll sagt, was es bedeutet. Ein neues agentisches Protokoll erfindet fast nie eine neue Leitung – MCP, A2A, AP2 und ACP laufen alle über HTTP und JSON. Neu ist die Bedeutungsebene: Tools, Tasks, Mandates, Agent Cards.

---

## 2. Die Landkarte: Wer spricht mit wem?

Bevor es in die einzelnen Protokolle geht, hilft eine Landkarte. Die agentischen Protokolle lassen sich am besten danach sortieren, **welche zwei Parteien** sie verbinden. Genau das ist auch die Logik, mit der die Projekte sich selbst voneinander abgrenzen: MCP verbindet einen Agenten mit Tools und Daten („vertikal“), A2A verbindet Agenten untereinander („horizontal“).[^a2a-mcp] AG-UI und A2UI verbinden den Agenten mit der UI vor dem Menschen. AP2, ACP und UCP verbinden Agenten mit Händlern (Merchants) und Payment Service Providern (PSPs), x402 einen aufrufenden Agenten mit einer bezahlpflichtigen Ressource.

```mermaid
flowchart TB
    U([Mensch]) -->|"AG-UI / A2UI / MCP Apps<br/>Events & UI"| H
    subgraph H[Host-Anwendung mit Orchestrierungs-Agent]
        direction TB
        O[Orchestrator-Agent]
    end
    O -->|"A2A<br/>Task delegieren"| S1[Spezialist-Agent<br/>Reise]
    O -->|"A2A<br/>Task delegieren"| S2[Spezialist-Agent<br/>Einkauf]
    ...
    B1 -.->|"Webhooks / Message Queue<br/>Events zurück"| H
    S2 -->|"AP2 / ACP / UCP<br/>Mandat, Warenkorb, Zahlung"| P["Händler (Merchants) &<br/>PSPs"]
    S2 -->|"x402<br/>Bezahlen pro Aufruf"| X[(Bezahlpflichtige API)]
```

Die Grafik zeigt das typische Zusammenspiel: Ein Mensch redet mit einer Host-Anwendung, in der ein Orchestrator-Agent läuft. Der delegiert Teilaufgaben per A2A an spezialisierte Agenten – die können intern völlig anders gebaut sein, von einem anderen Anbieter kommen und auf einer anderen Plattform laufen. Jeder Agent greift per MCP auf seine Tools zu. Die MCP-Server wiederum sind meist nur dünne Adapter vor den klassischen APIs, die es schon gab. Events aus den Bestandssystemen kommen per Webhook oder Queue zurück. Und sobald Geld fließt, kommen die Handels- und Zahlungsprotokolle ins Spiel.
...
<div class="maturity" role="img" aria-label="Reifegrad in drei Stufen: seit Jahrzehnten etabliert (REST, SOAP, Webhooks, WebSockets, SSE, gRPC, GraphQL, AMQP, MQTT, JSON-RPC), neu aber etabliert (MCP, A2A), experimentell (AG-UI, A2UI, MCP Apps, x402, AP2, ACP, UCP, Web Bot Auth).">
  <div class="tier is-etabliert">
    ...
    <div class="hint">Stabile Spezifikationen, breite Tooling, jede Betriebsfrage ist beantwortet.</div>
    <ul>
  ...
  <div class="tier is-experimentell">
    <h4>Experimentell</h4>
    <div class="hint">Öffentliche Spezifikationen, teils echte Produkte dahinter, aber noch keine Version 1.0, keine neutrale Governance oder erst wenige Monate alt.</div>
    <ul>
...
Die Einstufung „experimentell“ ist keine Abwertung. Hinter UCP steht der laufende Checkout in Google-Surfaces, hinter ACP stand mit Instant Checkout in ChatGPT ein echtes Produkt, hinter x402 laufen Millionen Transaktionen. Aber wer heute eine Architektur auf eines dieser Protokolle stellt, muss mit Breaking Changes rechnen und sollte sie kapseln. Bei MCP und A2A ist dieses Risiko deutlich kleiner geworden, aber auch nicht null: Die MCP-Revision vom Juli 2026 hat den Kern des Protokolls stateless gemacht und mehrere Primitives als deprecated markiert (dazu in Abschnitt 4).
...
<div class="proto-grid wide">
  <article class="proto-card is-etabliert">
    ...
    <dl>
      <dt>Ebene</dt><dd>Message-Konvention über HTTP</dd>
      <dt>Format</dt><dd>JSON (früher XML)</dd>
      <dt>Muster</dt><dd>Request/Response, stateless, Ressourcen + HTTP-Verben</dd>
      <dt>Auth</dt><dd>OAuth 2.0/2.1, API-Keys, mTLS – nicht vorgeschrieben</dd>
    </dl>
    <ul>
      <li><strong>Stärken:</strong> überall verstanden, cachebar, riesiges Tooling-Ökosystem, 93 % Nutzung laut Postman 2025</li>
  ...
  <article class="proto-card is-etabliert">
    ...
    <dl>
      <dt>Ebene</dt><dd>Message-Konvention (XML-Envelope)</dd>
      <dt>Format</dt><dd>XML, formal per XSD</dd>
      <dt>Muster</dt><dd>Request/Response, Operationen statt Ressourcen</dd>
      <dt>Auth</dt><dd>WS-Security: Signaturen und Verschlüsselung auf Message-Ebene</dd>
    </dl>
    <ul>
      <li><strong>Stärken:</strong> formale Verträge, Message-Level Security (nicht nur Transport-Level), Transaktionen</li>
  ...
  <article class="proto-card is-etabliert">
    ...
    <dl>
      ...
      <dt>Muster</dt><dd>Client bestimmt die Form der Antwort; ein Endpoint</dd>
  ...
  <article class="proto-card is-etabliert">
    ...
    <ul>
      ...
      <li><strong>Wann:</strong> interne Service-zu-Service-Kommunikation mit hohem Durchsatz; eine der drei A2A-Bindings</li>
    </ul>
  </article>
  <article class="proto-card is-etabliert">
    ...
    <dl>
      ...
      <dt>Muster</dt><dd>Event-Push statt Polling, „at least once“, Retries</dd>
      <dt>Auth</dt><dd>HMAC-Signatur + Timestamp im Header</dd>
    </dl>
    <ul>
      ...
      <li><strong>Wann:</strong> „Sag mir Bescheid, wenn…“ – Zahlungen, Deployments, Ticketänderungen; A2A nutzt sie für Push Notifications</li>
    </ul>
  </article>
  <article class="proto-card is-etabliert">
    ...
    <dl>
      <dt>Ebene</dt><dd>Transport (Anwendungsprotokoll)</dd>
    ...
    <ul>
      ...
      <li><strong>Schwächen:</strong> stateful Verbindungen erschweren Load Balancing</li>
  ...
  <article class="proto-card is-etabliert">
    ...
    <dl>
      ...
      <dt>Format</dt><dd>binär, Payload beliebig</dd>
      <dt>Muster</dt><dd>Publish/Subscribe, Queues, dauerhafte Event-Streams</dd>
      <dt>Auth</dt><dd>SASL, mTLS, Broker-ACLs</dd>
    </dl>
    <ul>
      <li><strong>Stärken:</strong> zeitliche Entkopplung, Pufferung, Fan-out, Replay</li>
      <li><strong>Schwächen:</strong> Betriebsaufwand, keine direkte Antwort</li>
      <li><strong>Wann:</strong> Events zwischen vielen Systemen; IoT (MQTT); Event-Historie (Kafka)</li>
    </ul>
  </article>
  <article class="proto-card is-etabliert">
    <div class="proto-head"><span class="proto-name">JSON-RPC 2.0</span><span class="status is-etabliert">Etabliert</span></div>
    <p class="proto-sub">2010, unverändert seit 2013 · die Basis von MCP und eine A2A-Binding</p>
    <dl>
      <dt>Ebene</dt><dd>Message-Konvention, transportunabhängig</dd>
      <dt>Format</dt><dd>JSON: <code>method</code>, <code>params</code>, <code>id</code></dd>
      <dt>Muster</dt><dd>Aufruf, Antwort, Notification (ohne <code>id</code>), Batch</dd>
...
sequenceDiagram
    ...
    C->>S: POST /auftrag (mit Callback-URL)
    S-->>C: 202 Accepted
    S->>C: POST https://client/hook (signiert)
```

Agenten arbeiten oft lange: Ein Task kann Sekunden oder Stunden dauern. Deshalb enthalten MCP und A2A alle drei Muster. MCP streamt Antworten per SSE und hat mit der Tasks-Extension ein Polling-Modell für lange Aufrufe; A2A kennt Polling (`GetTask`), Streaming (`SendStreamingMessage`) und Push Notifications an eine Webhook-URL.[^a2a-streaming] Wer die klassischen Muster versteht, erkennt sie in den neuen Protokollen wieder.

### Wann bleibt das Klassische die richtige Wahl?

Kurz gesagt: **fast immer, wenn kein LLM (Large Language Model) im Spiel ist.** Ein Batch-Job, der nachts Daten abgleicht, braucht kein MCP. Zwei Microservices, die sich Bestellungen zuschieben, brauchen kein A2A. Die agentischen Protokolle lösen ein spezifisches Problem – ein LLM soll zur Laufzeit *entdecken*, welche Capabilities es gibt, und sie mit natürlichsprachlicher Beschreibung auswählen. Wo der Aufrufer ein Programm ist, das genau weiß, was es will, ist eine REST-API mit OpenAPI-Beschreibung schneller, billiger, sicherer und besser zu betreiben.
...
MCP hat drei Rollen. Der **Host** ist die Anwendung, in der das Modell läuft (Claude Desktop, ein IDE, ein eigener Agent). Der Host erzeugt pro Verbindung einen **Client**, der genau mit einem **Server** spricht. Ein Server stellt Capabilities bereit: einen Datei-Zugriff, ein CRM, eine Datenbank, eine Suche. Der Host kann viele Server gleichzeitig eingebunden haben; das Modell sieht deren Tools in einer gemeinsamen Liste.
...
### Die Primitives: Was MCP ausmacht

Das Herz von MCP sind drei **Server-Primitives**, die sich darin unterscheiden, *wer* sie steuert:[^mcp-spec]

| Primitive | Wer steuert | Was es ist | Beispiel |
|---|---|---|---|
...
| **Prompts** | der Mensch | vorbereitete Prompt-Templates, die der Nutzer auswählt, oft mit Parametern | „Erstelle einen Wochenbericht für Kunde X“ |

Dazu kommt auf der Client-Seite die **Elicitation**: Ein Server kann während eines Aufrufs strukturierte Rückfragen an den Nutzer stellen („Welches Konto meinst du?“), seit der Revision 2025-11-25 auch als *URL-Modus*, der den Nutzer für einen Login oder ein Approval in den Browser schickt.[^mcp-2025-11] Zwei weitere Client-Primitives der Anfangszeit, **Roots** (welche Verzeichnisse darf der Server sehen) und **Sampling** (der Server bittet das Modell des Hosts um eine Completion), sind seit 2026-07-28 als *deprecated* markiert – mit mindestens zwölf Monaten Übergangsfrist.[^mcp-2026-07]

Rund um diesen Kern gibt es seit 2025-11-25 ein **Extension-Modell**. Die wichtigsten offiziellen Extensions:

- **Tasks** für lange laufende Aufrufe mit States `working`, `input_required`, `completed`, `failed`, `cancelled` und Polling per `tasks/get`.[^mcp-tasks]
- **MCP Apps**: Ein Server liefert eine kleine HTML-UI mit, die der Host in einer Sandbox rendert – gemeinsam mit OpenAI entwickelt und seit Januar 2026 offizielle Extension.[^mcp-apps]
- **Enterprise-Managed Authorization** und **Client Credentials** für Enterprise-Auth (Abschnitt 9).
...
<div class="proto-grid">
  <article class="proto-card is-neu">
    ...
    <dl>
      <dt>Verbindet</dt><dd>Agent ↔ Tools, Daten, Vorlagen</dd>
      <dt>Konvention</dt><dd>JSON-RPC 2.0</dd>
      <dt>Transport</dt><dd>stdio (lokal) · Streamable HTTP (remote)</dd>
      <dt>Kern</dt><dd>Tools · Resources · Prompts · Elicitation · Extensions (Tasks, Apps, Auth)</dd>
...
MCP definiert zwei Standardtransporte. **stdio** startet den Server als Subprocess und schickt JSON-RPC-Nachrichten zeilenweise über dessen Standard-Ein- und -Ausgabe – ideal für lokale Tools, die auf Dateien oder die Shell des Nutzers zugreifen. **Streamable HTTP** ist der Transport für entfernte Server: ein einziger HTTP-Endpoint, jede Nachricht ein POST, die Antwort wahlweise als einzelnes JSON-Objekt oder als SSE-Stream (`Content-Type: text/event-stream`).[^mcp-transports] Der ursprüngliche Transport „HTTP+SSE“ mit getrenntem Event-Endpoint ist deprecated.[^mcp-2026-07]

### Was sich 2026-07-28 geändert hat

Die Juli-Revision hat den Kern **stateless** gemacht: Der `initialize`-Handshake und die Session-ID `Mcp-Session-Id` sind weg; jede Anfrage trägt Protokollversion und Client-Capabilities selbst mit. Server-Notifications laufen über einen optionalen, long-lived Stream (`subscriptions/listen`). Rückfragen mitten in einem Tool-Aufruf werden nicht mehr als Server-initiierter Request gestellt, sondern über **Multi Round-Trip Requests**: Der Server antwortet mit `resultType: "input_required"`, der Client ruft mit den fehlenden Eingaben erneut auf. Zwei neue HTTP-Header, `Mcp-Method` und `Mcp-Name`, erlauben Gateways das Routing, ohne den JSON-Body zu parsen. Und Tasks wurden vom Kern in eine Extension verschoben.[^mcp-2026-07]

Für die Praxis heißt das: MCP ist reifer geworden und lässt sich hinter API-Gateways betreiben wie jede andere HTTP-API. Aber Tools, die auf Sessions, Roots oder Sampling gebaut haben, brauchen bis Mitte 2027 eine Anpassung.

### Autorisierung: der am stärksten standardisierte Teil

Kein anderes Protokoll in diesem Beitrag legt Authentifizierung und Autorisierung so genau fest wie MCP. Für HTTP-Transporte ist **OAuth 2.1** Pflicht, und zwar mit einer Kette bekannter IETF-Bausteine:[^mcp-auth]
...
sequenceDiagram
    ...
    A-->>C: RFC 8414 / OIDC Discovery: Endpoints, Capabilities
    Note over C,A: Client Registration per CIMD (client_id = HTTPS-URL)<br/>oder vorregistriert – Dynamic Client Registration ist deprecated
...
Die Regeln im Einzelnen: Der MCP-Server ist ein *Resource Server* und muss **Protected Resource Metadata (RFC 9728)** veröffentlichen, in der steht, welche Authorization Server für ihn zuständig sind. Der Client muss beide Discovery-Verfahren beherrschen (RFC 8414 und OpenID Connect Discovery), muss **PKCE** verwenden, muss per **Resource Indicator (RFC 8707)** sagen, für welchen Server das Token bestimmt ist – damit ein Token für Server A nicht bei Server B funktioniert –, und muss den `iss`-Parameter nach RFC 9207 prüfen. Für die Registration unbekannter Clients ist seit 2026-07-28 das **Client ID Metadata Document** der bevorzugte Weg: Die `client_id` ist eine HTTPS-URL, unter der der Client seine Metadaten veröffentlicht. Die ältere Dynamic Client Registration (RFC 7591) ist als deprecated markiert.[^mcp-auth] Für stdio-Server gilt das alles nicht: Sie holen Credentials aus der Umgebung, etwa aus Umgebungsvariablen.[^mcp-auth]

Für Unternehmen wichtig ist die Extension **Enterprise-Managed Authorization**, seit Juni 2026 als stabil erklärt. Sie setzt den IETF-Entwurf *Identity Assertion Authorization Grant* um, besser bekannt als **Cross App Access**: Der Nutzer loggt sich einmal beim Unternehmens-IdP ein, und der IdP stellt daraus Tokens für jeden freigegebenen MCP-Server aus – ohne dass für jeden Server ein eigener Consent-Dialog aufgeht und ohne dass die Fachabteilung im Alleingang Verbindungen anlegt. Okta, Anthropic, VS Code sowie Atlassian, Figma, Linear und weitere Server-Anbieter unterstützen das.[^mcp-ema]

### Die bekannten Schwächen

MCP hat ein Sicherheitsproblem, das kein Protokoll lösen kann: Tool-Beschreibungen sind Text, und Text liest das Modell als Anweisung. Invariant Labs hat im April 2025 gezeigt, wie ein bösartiger Server per **Tool Poisoning** versteckte Anweisungen in Beschreibungen legt, sie nach der Freigabe austauscht („Rug Pull“) oder die Tools anderer Server überschattet.[^invariant] Trail of Bits beschrieb wenig später das „Line Jumping“: Die Injektion wirkt schon beim Laden der Beschreibung, bevor ein einziges Tool aufgerufen wurde.[^tob] Das ist die Prompt Injection aus [meinem Beitrag über manipulierte Produktseiten](/blog/how-to-mislead-ai/), nur dass der Angreifer jetzt ein Tool statt einer Webseite ist. Die Konsequenz ist organisatorisch: MCP-Server sind Software von Dritten und brauchen dieselbe Freigabe wie jede andere Abhängigkeit.
...
Der Grundgedanke: Agenten sind **opak**. Sie tauschen Tasks und Ergebnisse aus, aber nicht ihre Gedanken, Pläne oder Tools. Ein Reise-Agent von Anbieter A kann einen Spesen-Agenten von Anbieter B beauftragen, ohne zu wissen, mit welchem Modell oder Framework der gebaut ist.[^a2a-spec]

### Die Kernkonzepte: Agent Card, Skills, Tasks, Artifacts
...
flowchart TB
    subgraph CARD["Agent Card · /.well-known/agent-card.json"]
        ...
        I[supportedInterfaces:<br/>URL + Binding + Version]
...
Die **Agent Card** ist die Visitenkarte: ein JSON-Dokument unter `/.well-known/agent-card.json`, das Name, Beschreibung, Anbieter, die angebotenen Interfaces (URL, Binding, Protokollversion), die Capabilities und die Security Schemes nennt. Sie kann per JWS signiert sein, damit ein Client prüfen kann, dass sie wirklich vom genannten Anbieter stammt, und es gibt eine **erweiterte Agent Card**, die erst nach Authentifizierung mehr Details preisgibt.[^a2a-spec]

**Skills** beschreiben, *was* ein Agent kann – mit ID, Name, Beschreibung, Tags, Beispielen und den unterstützten Ein- und Ausgabemodi. Das ist das Gegenstück zu MCP-Tools, aber gröber: Ein Skill ist „Reisen buchen“, kein einzelner Function Call mit Schema. Der Client-Agent entscheidet anhand der Skills, welchem Agenten er einen Task gibt, und formuliert den Task dann in natürlicher Sprache.

Die **Task** ist die Arbeitseinheit. Sie hat einen Lifecycle mit festen States:[^a2a-spec]
...
stateDiagram-v2
    ...
    WORKING --> AUTH_REQUIRED: Auth nötig
...
Innerhalb einer Task fließen **Messages**, die aus **Parts** bestehen – Text, Dateien oder strukturierte Daten; seit 1.0 ein vereinheitlichter Part-Typ. Ergebnisse kommen als **Artifacts** zurück, ebenfalls aus Parts. Fortschritt bekommt der Client auf drei Wegen: Polling per `GetTask`, Streaming per SSE oder **Push Notifications** an eine Webhook-URL, die er beim Start mitgibt.[^a2a-streaming] Version 1.0 hat außerdem `tasks/list` mit Cursor-Pagination und Multi-Tenancy ergänzt.[^a2a-v1]
...
<div class="proto-grid">
  <article class="proto-card is-neu">
    ...
    <dl>
      ...
      <dt>Konvention</dt><dd>drei Bindings: JSON-RPC 2.0, gRPC, HTTP+JSON/REST</dd>
      <dt>Transport</dt><dd>HTTP(S); Streaming per SSE, Push per Webhook</dd>
      <dt>Kern</dt><dd>Agent Card · Skills · Tasks mit States · Messages/Parts · Artifacts</dd>
...
### Drei Bindings, eine Semantik

Seit Version 0.3 (Juli 2025) ist A2A nicht mehr an JSON-RPC gebunden. Die Datentypen sind normativ in einer Protobuf-Datei definiert, und es gibt drei gleichwertige **Bindings**: JSON-RPC 2.0 über HTTP, gRPC und eine REST-artige HTTP+JSON-Variante.[^a2a-spec] Ein Agent nennt in seiner Card, welche er anbietet. Das ist ein bewusster Unterschied zu MCP, das auf JSON-RPC festgelegt ist – und es macht A2A für Umgebungen attraktiv, die intern längst auf gRPC laufen.

### Authentifizierung in A2A: deklariert, nicht vorgeschrieben

A2A geht bei der Authentifizierung den umgekehrten Weg von MCP. Statt ein Verfahren vorzuschreiben, **deklariert** jeder Agent in seiner Card, was er akzeptiert – mit denselben `securitySchemes`, die man aus OpenAPI kennt: API-Key, HTTP-Auth (etwa Bearer), OAuth 2 (Version 1.0 hat die Implicit- und Password-Flows gestrichen, den Device-Code-Flow ergänzt und ein `pkce_required`-Flag eingeführt), OpenID Connect oder mutual TLS.[^a2a-spec] Der Client muss sich dann auf HTTP-Ebene entsprechend ausweisen. Braucht der Agent mitten in einer Aufgabe zusätzliche Rechte – etwa für ein Drittsystem –, wechselt die Task in den State `AUTH_REQUIRED`.

Diese Flexibilität ist Stärke und Schwäche zugleich. Zwischen zwei Unternehmen, die ohnehin ihre IdPs federieren, ist sie genau richtig. Wer aber viele fremde Agenten anbindet, bekommt viele verschiedene Auth-Verfahren – und muss selbst dafür sorgen, dass ein Token für Agent A nicht bei Agent B landet. MCP hat diese Frage per RFC 8707 im Protokoll beantwortet; bei A2A ist es Sache der Implementierung.

---

## 6. MCP, A2A oder doch nur eine API? Das Zusammenspiel

Die Projekte selbst beschreiben das Verhältnis so: MCP gibt einem Agenten *Tiefe* (Zugriff auf Tools), A2A gibt einem System *Reichweite* (Zusammenarbeit zwischen Agenten).[^a2a-mcp] In der Praxis stellt sich aber meist eine dritte Frage: Braucht es überhaupt eines von beiden, oder reicht die vorhandene API?

| Frage | REST/GraphQL/gRPC direkt | MCP | A2A |
|---|---|---|---|
| Wer ruft auf? | ein Programm, das genau weiß, was es will | ein LLM, das zur Laufzeit auswählt | ein Agent, der einen Task delegiert |
| Was wird ausgetauscht? | Daten nach festem Vertrag | Tool Calls mit Schema, Ressourcen, Rückfragen | Tasks in natürlicher Sprache, Artifacts |
| Granularität | Funktion/Ressource | Funktion (Tool) | Capability (Skill) |
| Wer hält den State? | meist niemand (stateless) | seit 2026-07-28 der Aufrufer bzw. die Tasks-Extension | die Task auf dem Server-Agenten |
| Gegenstelle ist … | ein System | ein Adapter vor Systemen | ein eigenständiger, opaker Agent |
| Typische Latenz | Millisekunden | Sekunden (Modell + Tool) | Sekunden bis Stunden |

Drei Faustregeln, die sich aus der Tabelle ergeben:

1. **Ein Tool, das genau eine Sache tut, ist ein MCP-Tool, kein Agent.** Wer „Ticket anlegen“ als A2A-Agent baut, hat einen Agenten ohne Entscheidungsspielraum – teurer, langsamer und schwerer zu testen als ein Tool.
2. **Ein A2A-Agent lohnt sich, wenn die Gegenstelle selbst überlegt, mehrere Schritte braucht oder einem anderen Team gehört.** Die Opazität ist dann kein Nachteil, sondern das Interface: Das andere Team darf seinen Agenten umbauen, solange die Skills gleich bleiben.
3. **Ein MCP-Server ist fast immer ein Adapter vor einer bestehenden API.** Er ersetzt REST nicht, sondern übersetzt: aus zwanzig Endpoints werden fünf gut beschriebene Tools, aus Fehlercodes werden Texte, die ein Modell versteht, aus Pagination wird ein einziger Aufruf. Wer seine REST-API nur eins zu eins spiegelt, überfordert das Modell mit Auswahl und bekommt schlechtere Ergebnisse als mit einer kuratierten Tool-Menge.
...
## 7. Agent und UI: AG-UI, A2UI und MCP Apps

Eine Lücke blieb lange offen: Wie kommt das, was ein Agent tut, *auf den Bildschirm*? Ein Chat-Fenster, das Text streamt, reicht nicht, sobald der Agent Formulare, Tabellen, Approval-Dialoge oder Fortschritt anzeigen soll. Drei Ansätze konkurrieren – alle jung.
...
<div class="proto-grid wide">
  <article class="proto-card is-experimentell">
    ...
    <dl>
      ...
      <dt>Idee</dt><dd>ein <strong>Event-Stream</strong>: RUN_STARTED, TEXT_MESSAGE_CONTENT, TOOL_CALL_START/ARGS/END, STATE_SNAPSHOT/STATE_DELTA (JSON Patch) u. v. m.</dd>
    ...
    <ul><li><strong>Wann:</strong> eigene Web-UI über einem Agenten-Framework, die Zwischenstände, Tool Calls und Shared State live zeigen soll</li></ul>
  </article>
  <article class="proto-card is-experimentell">
    ...
    <dl>
      <dt>Verbindet</dt><dd>Agent ↔ Client-UI (Web, Flutter, Android/iOS geplant)</dd>
      <dt>Idee</dt><dd>der Agent beschreibt die <strong>UI deklarativ als JSON</strong>; der Client rendert aus einem Katalog nativer Komponenten – kein Code wird ausgeführt</dd>
      <dt>Transport</dt><dd>egal; als A2A-Extension in einem DataPart (<code>application/a2ui+json</code>), über AG-UI oder in MCP Apps</dd>
    </dl>
    <ul><li><strong>Wann:</strong> ein fremder Agent soll in der eigenen App UIs erzeugen, ohne dass man ihm HTML oder JavaScript erlaubt</li></ul>
  </article>
  <article class="proto-card is-experimentell">
    <div class="proto-head"><span class="proto-name">MCP Apps</span><span class="status is-experimentell">Experimentell</span></div>
    <p class="proto-sub">MCP-Extension (mit OpenAI), offiziell seit Jan 2026</p>
    <dl>
      <dt>Verbindet</dt><dd>MCP-Server ↔ Host-UI</dd>
      <dt>Idee</dt><dd>der Server liefert eine <strong>HTML-UI</strong> mit, die der Host in einem Sandbox-iframe anzeigt</dd>
...
Die Unterschiede sind leicht zu merken: AG-UI standardisiert den **Strom** zwischen Backend und Frontend (was gerade passiert), A2UI standardisiert die **Beschreibung** der UI (was angezeigt werden soll), MCP Apps liefern **fertiges HTML** in einer Sandbox. Die drei schließen sich nicht aus – A2UI-Nachrichten können in AG-UI-Events transportiert werden.[^a2ui] Alle drei sind für die Bewertung „experimentell“: AG-UI ist noch bei Version 0.0.x,[^agui] A2UI hat seine 1.0 als Release Candidate mit Ziel Ende 2026,[^a2ui] MCP Apps ist wenige Monate alt.[^mcp-apps] Wer heute eine UI baut, sollte den Zugriff auf diese Protokolle in einer eigenen Schicht kapseln.

---

## 8. Agentic Payments und Agentic Commerce: x402, AP2, ACP, UCP
...
sequenceDiagram
    ...
    Note over A: wählt Angebot, signiert Payment Authorization<br/>(EIP-3009, Nonce, Gültigkeitsfenster)
...
- **Kein Account, kein Login, kein Abrechnungsvertrag.** Der Agent zahlt pro Aufruf mit einem Stablecoin (in der Praxis fast immer USDC) aus seiner Wallet. Das macht x402 zur ersten praktikablen Lösung für **Mikrozahlungen zwischen Maschinen** – ein API-Aufruf für einen Zehntelcent.
- **Version 2 seit Dezember 2025**: Die Payment Requirements wanderten aus dem Body in den Base64-kodierten Header `PAYMENT-REQUIRED`, die Signatur heißt `PAYMENT-SIGNATURE`, die Settlement-Bestätigung `PAYMENT-RESPONSE`.[^x402-v2] Die Spezifikation ist transportunabhängig und definiert Bindings für HTTP, MCP und A2A.[^x402-spec]
- **Schemes**: `exact` (fester Betrag), `upto` (Obergrenze, tatsächlicher Verbrauch wird abgerechnet), Batch-Settlement mit Gutscheinen. Der Facilitator kann Betrag und Empfänger nicht ändern, weil beides in der signierten Authorization steckt.[^x402-spec]
...
- **Keine Chargebacks.** Eine bestätigte Transaktion ist final; Chargebacks, Disputes oder Refunds muss der Anbieter selbst anbieten.
- **Der Facilitator ist eine Trusted Party außerhalb des Protokolls.** Eine auf der USENIX Security 2026 vorgestellte Untersuchung von 15 Facilitators fand in allen Verstöße und vier Angriffsklassen: Free Shopping, Asset Theft, Service Denial und Gas Abuse (kostenloser Bezug, Diebstahl von Assets, Blockieren des Dienstes, Missbrauch der Gas Fees). Die Lücken wurden gemeldet, Coinbase hat Gegenmaßnahmen übernommen.[^usenix-x402] Eine weitere Arbeit zeigte, dass `/verify` stateless ist und einen Nachweis nicht reserviert, sodass er bei gleichzeitigen Anfragen mehrfach verwendet werden konnte.[^freeride]
...
Das **Agent Payments Protocol** hat Google am 16. September 2025 mit über sechzig Partnern vorgestellt – Adyen, American Express, Coinbase, Etsy, Mastercard, PayPal, Revolut, Salesforce, ServiceNow, UnionPay, Worldpay und andere.[^ap2-launch] AP2 ist bewusst Payment-Method-agnostisch: Karten, Echtzeitüberweisungen und Stablecoins sind alle vorgesehen; für Krypto gibt es eine gemeinsam mit Coinbase gebaute A2A-x402-Extension.[^a2a-x402]

Der Kern von AP2 sind **Mandate**: kryptografisch signierte, überprüfbare Nachweise darüber, was der Nutzer erlaubt hat. In der Startversion hießen sie Intent Mandate (Vorab-Auftrag mit Grenzen, für den Fall „Human Not Present“), Cart Mandate (der Nutzer signiert den exakten Warenkorb) und Payment Mandate (verknüpft die Zahlungsart mit dem Kauf).[^ap2-launch] Die aktuelle Spezifikation v0.2 vom April 2026 hat das Modell auf zwei Mandate-Typen zusammengezogen, jeweils in einem *offenen* State (open: mit Bedingungen, vom Nutzer signiert) und einem *geschlossenen* State (closed: final): das **Checkout Mandate** als Nachweis gegenüber dem Händler und das **Payment Mandate** als Nachweis gegenüber Wallet, Zahlungsnetz und Payment Processor des Händlers. Technisch sind es SD-JWT Verifiable Credentials mit Key Binding, wie sie aus der digitalen Identität bekannt sind.[^ap2-spec]
...
sequenceDiagram
    ...
    participant PP as Payment Processor / Netz
    U->>SA: „Kauf mir Laufschuhe Größe 43, max. 150 €“
    Note over U,SA: Human Not Present: Nutzer signiert OFFENES Checkout Mandate<br/>(Bedingungen: Artikel, Preisgrenze, erlaubte Händler)
...
Was AP2 ausmacht: Es beantwortet die Frage nach **Autorisierung, Authentizität und Verantwortlichkeit** mit einem nicht abstreitbaren Beleg, den alle Beteiligten – Händler, Wallet, Kartennetzwerk, Bank – prüfen können. Der Unterschied „Human Present“ (der Nutzer bestätigt den finalen Warenkorb) versus „Human Not Present“ (der Agent schließt innerhalb vorab signierter Grenzen selbst ab) ist explizit im Protokoll abgebildet.[^ap2-spec] Und AP2 ist kein eigener Transport, sondern läuft als Extension über A2A, MCP oder UCP.

Zur Reife: AP2 steht bei Version 0.2. Am 28. April 2026 hat Google das Protokoll an die **FIDO Alliance** übergeben, Mastercard hat sein kompatibles „Verifiable Intent“ beigesteuert. Die FIDO Alliance hat dafür zwei Arbeitsgruppen gegründet – eine zur Agenten-Authentifizierung mit Chairs von CVS Health, Google und OpenAI, eine zu Payments unter Vorsitz von Mastercard und Visa.[^fido] Das ist bemerkenswert: Es ist das erste Gremium, in dem OpenAI, Google und beide Kartennetzwerkwerke gemeinsam an einem Zahlungsstandard für Agenten arbeiten. Produktiv genutzt wird AP2 heute vor allem als Mandat-Extension von UCP (Abschnitt 8.4).
...
ACP besteht aus mehreren Teilen, die zusammen den Weg von der Product Discovery bis zur Bestellung abdecken (aktueller Stand der Spezifikation: 2026-04-17):[^acp-repo]

- Ein **Produkt-Feed**, mit dem Händler ihren Katalog an die KI-Plattform liefern.
- Eine **Checkout-API** mit Checkout Sessions: create, update, complete, cancel – der Händler bleibt Betreiber des Checkouts und **Merchant of Record**; die Plattform ist nie Vertragspartner des Kaufs.
- **Delegated Payment**: Der PSP des Nutzers stellt ein einmal nutzbares, auf Betrag, Währung, Händler, Session und Ablaufzeit begrenztes Token aus (bei Stripe „Shared Payment Token“), das der Händler über seinen eigenen PSP einlöst. Die Kartendaten verlassen die Wallet nie.
- Seit 2026: Discovery über `/.well-known/acp.json`, verpflichtender Idempotency-Key, Cart-API, 3-D-Secure-Delegation, Order-Objekte mit Refunds und Returns und eine **MCP-Binding** mit fünf Checkout-Tools.

```mermaid
sequenceDiagram
    participant U as Nutzer:in
    participant P as KI-Plattform (Agent)
    participant M as Händler (ACP-Endpoints)
...
Der wichtigste Punkt für die Einordnung: Das Protokoll lebt, das Produkt hat sich verändert. Im März 2026 hat OpenAI Instant Checkout zurückgenommen – die erste Version habe „nicht die Flexibilität geboten, die wir anstreben“ – und lässt Händler seitdem ihre eigene Checkout-Erfahrung in ChatGPT einbetten, während OpenAI sich auf die Product Discovery konzentriert.[^acp-retire] Berichte nennen als Gründe eine geringe Zahl angebundener Händler und schwache Abschlussquoten; das sind Sekundärquellen. Die ACP-Spezifikation ist danach trotzdem weiter gewachsen (die Revision 2026-04-17 kam nach der Rücknahme), PayPal hat sich per Partnerschaft angebunden, Händler wie Target und Sephora nutzen ACP Berichten zufolge für die Product Discovery, und Visa hat im Juni 2026 eine Zusammenarbeit mit OpenAI angekündigt, die Ausgabenlimits und Merchant-Whitelists des Nutzers in ChatGPT bringen soll.[^paypal-acp]

### 8.4 UCP – der Google-Weg, mit den Händlern

Das **Universal Commerce Protocol** hat Google am 11. Januar 2026 auf der Handelsmesse NRF vorgestellt, gemeinsam mit Shopify, Etsy, Wayfair, Target und Walmart entwickelt und von über zwanzig weiteren Unternehmen unterstützt, darunter Adyen, American Express, Best Buy, Mastercard, Stripe, Visa und Zalando.[^ucp-launch] UCP ist der Versuch, den *gesamten* Commerce-Flow zu standardisieren, nicht nur die Zahlung: Catalog Search, Warenkorb, Checkout, Identity Linking per OAuth 2.0, Order-Lifecycle mit Webhooks und Payment Token Exchange. Es ist modular aus **Capabilities** und **Extensions** aufgebaut und läuft über REST, MCP oder A2A. Die AP2-Mandate sind als Extension `dev.ucp.shopping.ap2_mandate` eingebunden.[^ucp-repo]

Anders als ACP ist UCP vom ersten Tag an produktiv: Der Checkout im KI-Modus der Google-Suche und in der Gemini-App läuft in den USA über UCP und Google Pay; Händler binden sich über das Merchant Center an. Im Mai 2026 kündigte Google den „Universal Cart“ über Suche, Gemini, YouTube und Gmail an, dazu Hotels, Lieferdienste und die Ausweitung auf Kanada, Australien und Großbritannien.[^ucp-gml] Die Spezifikation wird schnell weiterentwickelt: Die Version 2026-08-25 brachte Vertical-Extensions für Food & Lodging, Ratenzahlung, Loyalty-Programme und eine Überarbeitung des Buyer-Consent – mit Breaking Changes.[^ucp-repo]

### 8.5 Die Kartennetzwerkwerke: Visa, Mastercard und der Nachweis, wer da einkauft

Parallel zu den Protokollen der Plattformen bauen die Kartennetzwerkwerke ihre eigene Infrastruktur. Sie beantworten eine Frage, die keines der vier Protokolle allein löst: **Wie erkennt ein Händler, dass der Besucher ein legitimer Agent ist und nicht ein Bot?**

- **Visa Intelligent Commerce** (April 2025) führt „AI-Ready Cards“ ein – tokenisierte Kartendaten, die an einen Agenten gebunden werden.[^visa-ic] Das **Trusted Agent Protocol** (Oktober 2025, mit Cloudflare) lässt Agenten ihre HTTP-Anfragen nach **RFC 9421 HTTP Message Signatures** signieren; Händler prüfen die Signatur gegen ein von Visa betriebenes Key Directory und bekommen dabei Absicht des Agenten und Wiedererkennung des Kunden mitgeliefert.[^visa-tap]
- **Mastercard Agent Pay** (April 2025) arbeitet mit „Agentic Tokens“: Ein Kartentoken wird an Agent, Händler und den erteilten Consent gebunden.[^mc-agentpay] Mastercard hat sein „Verifiable Intent“ gemeinsam mit AP2 in die FIDO Alliance eingebracht[^fido] und im Juni 2026 „Agent Pay for Machines“ für Zahlungen zwischen Maschinen vorgestellt, das mit x402 und MPP kompatibel sein soll.[^mc-machines]
- Die technische Basis für die Agent-Verifikation ist **Web Bot Auth**: ein Satz IETF-Entwürfe von Cloudflare und Google, seit Oktober 2025 mit eigener Arbeitsgruppe, bei dem sich ein Agent mit einem Ed25519-Schlüssel ausweist, der unter `/.well-known/http-message-signatures-directory` veröffentlicht ist.[^webbotauth] Cloudflare setzt es produktiv ein; die Dokumente sind noch individuelle Entwürfe.

### 8.6 Vergleich und Einordnung

| | x402 | AP2 | ACP | UCP |
|---|---|---|---|---|
| **Governance** | x402 Foundation (Linux Foundation), von Coinbase | FIDO Alliance, von Google | OpenAI + Stripe | Google + Shopify, Etsy, Wayfair, Target, Walmart |
...
| **Geld** | Stablecoins on-chain | agnostisch (Karte, Überweisung, Krypto via x402) | Fiat über delegiertes Token beim PSP | Fiat über Google Pay und Payment-Handler |
...
| **Streitfälle** | keine, endgültig | bestehende Netzwerk-Regeln, Mandate als Beweis | bestehende Card-Network-Regeln, Refund- und Return-Objekte | bestehende Regeln, Bestell-Capability |
| **Transport** | HTTP-Header; Bindings für MCP, A2A | Extension über A2A, MCP, UCP | REST + Webhooks; MCP-Binding | REST, MCP, A2A |
| **Reife (Sep 2026)** | produktiv für Mikrozahlungen, geringe Volumina, Facilitator-Angriffe belegt | v0.2, Standardisierung bei FIDO begonnen | Spezifikation aktiv, Vorzeigeprodukt zurückgenommen | produktiv in Google-Surfaces (USA), schnelle Breaking Changes |

Meine Einordnung: Die vier stehen weniger in Konkurrenz, als es die Ankündigungen vermuten lassen. x402 und AP2 lösen orthogonale Probleme (Zahlen ohne Konto versus Beweis der Absicht) und werden gemeinsam eingesetzt. ACP und UCP sind die Handelsprotokolle *der jeweiligen Plattform* – wer in ChatGPT verkaufen will, kommt an ACP nicht vorbei, wer in Google-Surfaces verkaufen will, nicht an UCP. Die PSPs überbrücken das: PayPal und Stripe sprechen beide,[^paypal-acp] Visa und Mastercard sitzen in allen Gremien. Für ein Unternehmen, das heute plant, ist deshalb nicht die Protokollwahl entscheidend, sondern die Frage, ob der eigene Checkout, der Katalog und die Bestellprozesse **so gebaut sind, dass sich jede dieser Bindings als dünne Schicht davorlegen lässt**: sauber getrennte Preisberechnung, idempotente Bestellanlage, Zahlungstoken statt Kartendaten, ein Bestellstatus, der per Webhook nach außen geht. Genau das sind die klassischen API-Tugenden aus Abschnitt 3.
...
| **AG-UI / A2UI** | nichts – Sache des Transports | Session der Web-App |
...
| **AP2** | SD-JWT Verifiable Credentials mit Key Binding; Delegation per OpenID4VP oder Trusted Agent Provider[^ap2-spec] | Wallet/Passkey des Nutzers auf einer vertrauenswürdigen UI |
...
| **UCP** | Identity Linking per OAuth 2.0 (Scopes); Signaturen für Nachrichten[^ucp-repo] | Google-Konto ↔ Händlerkonto |
| **Web Bot Auth / Visa TAP** | RFC 9421 HTTP Message Signatures mit veröffentlichtem Schlüssel[^webbotauth] | Cloudflare-Verifikation, Visa-Key Directory |

Der Befund ist eindeutig: **MCP ist das am stärksten standardisierte Protokoll dieser Liste**, was Authentifizierung angeht. Es schreibt nicht nur OAuth vor, sondern legt bis ins Detail fest, wie Discovery, Client Registration und Token-Binding funktionieren. Das ist ein Grund, warum MCP in Unternehmensumgebungen so schnell akzeptiert wurde: Die IAM-Abteilung erkennt jeden Baustein wieder. A2A ist flexibler, verlagert aber die Verantwortung auf den Implementierer. Und die klassischen Protokolle haben nie etwas vorgeschrieben – REST-APIs mit Basic Auth und unbefristeten API-Keys gibt es bis heute zuhauf.

### 9.2 Das eigentliche Problem: Im Auftrag von wem?

Die klassische Frage in Service-Landschaften lautet: Ruft Dienst B den Dienst C als *sich selbst* auf (Client Credentials, Service-Identität) oder *im Auftrag* des Nutzers, der A angestoßen hat? Bei Agenten wird sie zur zentralen Sicherheitsfrage, weil ein Agent Tools aufruft, die er selbst ausgewählt hat – auf Basis von Text, der manipuliert sein könnte.
...
- **Delegation**: Der Agent bekommt ein eigenes, eingeschränktes Token, in dem steht, *für wen* und *wofür* er handelt. Das ist der Zweck von **Token Exchange (RFC 8693)**: Ein Token wird gegen ein anderes mit anderer Zielgruppe und anderem Umfang eingetauscht, und das Ergebnis trägt einen `act`-Claim, der die Delegation Chain dokumentiert.[^rfc8693]

Genau hier setzt die aktuelle Standardisierungsarbeit an. Der IETF-Entwurf **Identity Assertion Authorization Grant** (Okta, Ping; Entwurf 04 vom Mai 2026) – die Basis von MCPs Enterprise-Managed Authorization – lässt den Unternehmens-IdP aus einem Login Tokens für Drittdienste ausstellen, ohne Nutzer-Passwörter oder Consent-Dialoge pro Dienst.[^idjag] Der Entwurf **Identity and Authorization Chaining Across Domains** (Entwurf 17, Juli 2026) beschreibt dasselbe über Vertrauensgrenzen hinweg – also genau den A2A-Fall.[^idchain] Beide sind Arbeitsgruppen-Dokumente, keine RFCs. Wer heute baut, sollte Delegation mit RFC 8693 umsetzen und den Agenten als *eigenen* Principal führen: mit eigener Identität, eigenen Scopes und einem Audit-Log, das den Menschen dahinter nennt.
...
| **Long-running Operations** | 202 Accepted + Polling oder Webhook | Streaming per SSE; Tasks-Extension mit States[^mcp-tasks] | Task-Lifecycle, Streaming, Push Notifications[^a2a-streaming] |
| **Error-Handling** | HTTP-Statuscodes, Problem Details (RFC 9457) | JSON-RPC-Fehler plus `isError` im Tool-Ergebnis, damit das Modell lesbaren Text bekommt | `google.rpc.Status`-Fehler; States FAILED/REJECTED |
| **Versionierung** | URL-Pfad, Header, Semantic Versioning | datierte Revisionen, seit 2026-07-28 mit 12-Monats-Deprecation-Policy[^mcp-2026-07] | `protocolVersion` je Interface in der Card |
| **Observability** | Logs, Traces, Metriken pro Aufruf | `Mcp-Method`/`Mcp-Name`-Header für Gateways; sonst Sache des Hosts | Task-IDs und Kontext-IDs als Korrelation |
...
**Teil 1 – kein LLM als Aufrufer:**

```mermaid
flowchart TD
    START([Ich will zwei Systeme verbinden]) --> Q1{Ist ein LLM<br/>der Aufrufer?}
    ...
    Q3 -- "Ja, einzelne Events" --> WH[Webhooks<br/>mit Standard-Webhooks-Signatur]
...
**Teil 2 – ein LLM ist der Aufrufer:**

```mermaid
flowchart TD
    Q7{Ist die Gegenstelle<br/>selbst ein Agent?}
    Q7 -- "Nein: Tool, Daten, API" --> MCP[MCP-Server<br/>als kuratierter Adapter<br/>vor der REST-API]
    Q7 -- "Ja: eigenständig, opak,<br/>anderes Team / Anbieter" --> A2A[A2A<br/>Agent Card + Skills]
    MCP --> Q8{Fließt Geld?}
    A2A --> Q8
    Q8 -- Nein --> Q9{Braucht der Agent<br/>eine UI?}
    ...
    Q10 -- "eigene / mehrere" --> AP2[AP2-Mandate über A2A oder MCP<br/>+ PSP,<br/>der ACP und UCP spricht]
```

Ein paar Ergänzungen zum Baum, die ein Diagramm nicht tragen kann:

- **„Ist ein LLM der Aufrufer?“** ist die entscheidende Weiche. Sie ist mit Nein zu beantworten, wenn die Logik deterministisch ist – auch dann, wenn ein Modell *irgendwo* im System steckt. Ein Agent, der am Ende einen Bestellauftrag an die Warenwirtschaft übergibt, sollte das über die bestehende REST-API tun, nicht über einen zweiten Agenten.
- **MCP vor der eigenen API** heißt: kuratieren. Fünf Tools mit guten Beschreibungen schlagen fünfzig automatisch generierte.
- **A2A über Organisationsgrenzen** funktioniert nur mit geklärter Identität. Bevor man die erste Agent Card veröffentlicht, sollte feststehen, welcher IdP welche Tokens ausstellt und wie die Delegation Chain im Audit-Log aussieht (Abschnitt 9.2).
- **Bei Geld gilt: Bindings kapseln.** Jedes der vier Zahlungsprotokolle hat 2026 Breaking Changes gehabt. Ein Checkout, der sauber in Katalog, Warenkorb, Zahlung und Bestellung getrennt ist, überlebt sie; einer, der um ein bestimmtes Protokoll herumgebaut ist, nicht.

---

## 11. Fazit

Die agentischen Protokolle erfinden die Kommunikation nicht neu. Sie laufen über dieselben Leitungen wie alles andere – HTTP, JSON, TLS – und übernehmen die Muster, die REST, Webhooks und Message Queues seit Jahrzehnten vorleben: Request und Response, Streaming, Push, State Machines für Long-running Operations. Neu ist die Bedeutungsebene: **Tools mit Beschreibung**, die ein Modell zur Laufzeit auswählt (MCP); **Tasks und Skills**, die ein Agent an einen anderen delegiert, ohne dessen Innenleben zu kennen (A2A); **Mandate**, die beweisen, dass ein Mensch einen Kauf wollte (AP2); und ein **HTTP-Statuscode**, der nach dreißig Jahren endlich eine Bedeutung bekommt (x402).
...
[^a2a-streaming]: A2A Project, „Streaming & Asynchronous Operations“ (SSE-Streaming, Polling, Push Notifications an Webhooks). *Abgerufen 11.09.2026.* <https://a2a-protocol.org/latest/topics/streaming-and-async/>
...
[^google-mcp]: Google Cloud Blog, „Announcing official MCP support for Google services“ (Managed Remote-MCP-Server für Maps, BigQuery, Compute Engine, GKE). *Veröffentlicht 10.12.2025; abgerufen 11.09.2026.* <https://cloud.google.com/blog/products/ai-machine-learning/announcing-official-mcp-support-for-google-services>
...
[^mcp-2025-11]: Model Context Protocol Blog, „One Year of MCP: November 2025 Spec Release“ (Tasks, CIMD, URL-Elicitation, Extension-Modell, Registry-Zahlen). *Veröffentlicht 25.11.2025; abgerufen 11.09.2026.* <https://blog.modelcontextprotocol.io/posts/2025-11-25-first-mcp-anniversary/>

[^mcp-2026-07]: Model Context Protocol Blog, „The 2026-07-28 Specification“ und zugehöriges Changelog (statelessr Kern, Multi Round-Trip Requests, `Mcp-Method`/`Mcp-Name`, Tasks als Extension, Deprecation von Roots, Sampling, Logging, HTTP+SSE und Dynamic Client Registration). *Veröffentlicht 28.07.2026; abgerufen 11.09.2026.* <https://blog.modelcontextprotocol.io/posts/2026-07-28/> · Changelog: <https://modelcontextprotocol.io/specification/2026-07-28/changelog>

[^mcp-tasks]: Model Context Protocol, Extension „Tasks“ (`io.modelcontextprotocol/tasks`: States, `tasks/get`, `tasks/update`, `tasks/cancel`). *Abgerufen 11.09.2026.* <https://modelcontextprotocol.io/extensions/tasks/overview>
...
[^mcp-auth]: Model Context Protocol, Spezifikation 2026-07-28, „Authorization“ und „Client Registration“ (OAuth 2.1 draft-13, RFC 9728, RFC 8414, OIDC Discovery, PKCE, RFC 8707, RFC 9207, Client ID Metadata Documents; stdio-Server holen Credentials aus der Umgebung). *Veröffentlicht 28.07.2026; abgerufen 11.09.2026.* <https://modelcontextprotocol.io/specification/2026-07-28/basic/authorization>
...
[^a2a-spec]: A2A Project, „Agent2Agent (A2A) Protocol Specification“, Version 1.0 (Agent Card unter `/.well-known/agent-card.json`, Skills, Task-States, Parts, Artifacts, drei Bindings, `securitySchemes`, JWS-Signaturen, erweiterte Agent Card). *Veröffentlicht 12.03.2026 (v1.0.1: 28.05.2026); abgerufen 11.09.2026.* <https://a2a-protocol.org/latest/specification/>

[^a2a-v1]: A2A Project, „What's New in A2A v1.0“ (vereinheitlichter Part-Typ, `supportedInterfaces`, `tasks/list`, Multi-Tenancy, `google.rpc.Status`). *Veröffentlicht März 2026; abgerufen 11.09.2026.* <https://a2a-protocol.org/latest/whats-new-v1/>

[^agui]: AG-UI Protocol (CopilotKit), Repository und Event-Referenz (Lifecycle-, Text-, Tool-Call-, State-Events; transportagnostisch; Integrationen LangGraph, CrewAI, Mastra, Google ADK, Microsoft Agent Framework, AWS Strands). Aktuelle Paketversion `@ag-ui/core` 0.0.59. *Start Mai 2025; abgerufen 11.09.2026.* <https://github.com/ag-ui-protocol/ag-ui> · <https://docs.ag-ui.com/concepts/events>

[^a2ui]: A2UI Project (Google), Spezifikation v1.0 (Release Candidate), Roadmap und A2A-Extension (Transport von A2UI-Nachrichten als A2A-DataPart mit `application/a2ui+json`; Übersetzung in AG-UI-Events). *Start 15.12.2025; abgerufen 11.09.2026.* <https://github.com/a2ui-project/a2ui> · <https://a2ui.org/>
...
[^x402-spec]: x402 Foundation, „x402 Specification v2“, Schemes (`exact` mit EIP-3009, `upto`, Batch-Settlement) und Extensions (u. a. HTTP Message Signatures), Repository. *Abgerufen 11.09.2026.* <https://github.com/x402-foundation/x402/blob/main/specs/x402-specification-v2.md>
...
[^freeride]: „Free-Riding the Agentic Web“ (arXiv 2605.30998; statelesss `/verify`, fehlende Nonce-Reservierung, Mehrfachverwendung von Nachweisen bei gleichzeitigen Anfragen). *Veröffentlicht Mai 2026; abgerufen 11.09.2026.* <https://arxiv.org/abs/2605.30998>
...
[^ap2-launch]: Google Cloud Blog, „Announcing Agent Payments Protocol (AP2)“ (über 60 Partner; Intent-, Cart- und Payment-Mandate; Extension von A2A und MCP). *Veröffentlicht 16.09.2025; abgerufen 11.09.2026.* <https://cloud.google.com/blog/products/ai-machine-learning/announcing-agents-to-payments-ap2-protocol>
...
[^acp-repo]: Agentic Commerce Protocol, Repository mit Spezifikation (aktuelle Revision 2026-04-17: Checkout-Sessions, Delegated Payment mit Allowance, Feed, `/.well-known/acp.json`, MCP-Binding), Changelog und Governance. *Abgerufen 11.09.2026.* <https://github.com/agentic-commerce-protocol/agentic-commerce-protocol> · <https://www.agenticcommerce.dev/>
...
[^ucp-launch]: Google Developers Blog, „Under the Hood: Universal Commerce Protocol (UCP)“ (Vorstellung auf der NRF 2026; Mitentwickler Shopify, Etsy, Wayfair, Target, Walmart; Capabilities und Extensions; AP2-Mandat-Extension). *Veröffentlicht Januar 2026; abgerufen 11.09.2026.* <https://developers.googleblog.com/under-the-hood-universal-commerce-protocol-ucp/>
...
[^visa-tap]: Visa, „Visa Unveils Trusted Agent Protocol for AI Commerce“ (mit Cloudflare; RFC 9421 HTTP Message Signatures; Key Directory; 12 Startpartner). *Veröffentlicht 14.10.2025; abgerufen 11.09.2026.* <https://corporate.visa.com/en/sites/visa-perspectives/newsroom/visa-unveils-trusted-agent-protocol-for-ai-commerce.html> · Spezifikation: <https://github.com/visa/trusted-agent-protocol>
...
[^idjag]: A. Parecki, K. McGuinness, B. Campbell, „Identity Assertion Authorization Grant“, draft-ietf-oauth-identity-assertion-authz-grant-04, IETF OAuth Working Group (Basis von „Cross App Access“ und der MCP-Extension Enterprise-Managed Authorization). *Entwurf vom 21.05.2026; abgerufen 11.09.2026.* <https://datatracker.ietf.org/doc/draft-ietf-oauth-identity-assertion-authz-grant/>
